...
#加载模型
import build_model_prompt
model = build_model_prompt.build_ccim_model_VPT(num_classes=100, img_size=[224,224], base_model='vit_base_patch16_224_in21k', model_idx='ViT', patch_size=16,Prompt_Token_num=10, VPT_type="Deep",ccim=ccim,confounder=confounder,confounder2 = None, prob=prob,opt=opt) 
model = build_model.get_updateModel(model,'result/ratio0.1/VPT-TDE/datset=CIFAR100~net_v=vit_224~method=...~bs=64~decay=4~lr=0.01~lrd_rate=0.1~drop=0.0/model_best.pt')
model.cuda()
import build_model
model =model.cuda()
...
#     optimizer,warm_lr = build_model.set_optimizer(model,opt)
...
def train_epoch(epoch, decay, optimizer, modality):
    ...
    for batch_idx, (data, label, indexes) in enumerate(dataloadder['train']):
        ...
        if CUDA:
            ...
            label = label.cuda()
        #perform prediction
        feat,output = model(data1)
        featF,outputF = model(dataF)
        #compute loss
        ...
        if opt.rebalance=='focal':
            ...
            criterion_align = nn.MSELoss(reduction='mean')
            criterion1 = nn.CrossEntropyLoss(weight=weight_cls)
            criterion2 = nn.CrossEntropyLoss(weight=weight_cls)
            loss_cls_v = criterion1(output, label)
            loss_cls_v2 = criterion2(outputF, label)
            loss_cls_v3 = criterion_align(feat, featF)
        #可调节损失函数
        final_loss = 1.5*loss_cls_v + loss_cls_v2 + loss_cls_v3
...
if __name__ == '__main__':
    ...
    for epoch in range(1, EPOCHS):        
        ...
        if measure_cur > measure_best:
            ...
            epoch_best = epoch
            np.save(result_path + '/model_best.npy', model.get_embed())
        
        if epoch == EPOCHS:
            torch.save(model.state_dict(), result_path + '/model_final.pt')
            np.save(result_path+ '/model_final.npy', model.get_embed())
